...
): { isValid: true } | { isValid: false; reason: "drift_violation" } {
  if (!route.policy || !route.policy.drift_strict) return { isValid: true } as const;
  const sameProvider = route.provider.type === intended.provider;
  const sameModel = route.provider.model === intended.model;
  return sameProvider && sameModel ? ({ isValid: true } as const) : ({ isValid: false, reason: "drift_violation" } as const);
}
