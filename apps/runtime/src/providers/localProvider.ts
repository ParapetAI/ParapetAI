...
function parseSSEChunk(chunk: string): OpenAICompatibleResponse | null {
  if (!chunk.startsWith("data: ")) return null;
  const data = chunk.slice(6);
  if (data === "[DONE]") return null;
  try {
    return JSON.parse(data) as OpenAICompatibleResponse;
  } catch {
    return null;
  }
}
...
export const localProvider: ProviderAdapter = {
  ...
  async callLLM(input: LlmCallInput): Promise<LlmCallOutput> {
    if (!input.endpoint) {
      throw new Error("Local provider requires endpoint URL");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message ?? `Local provider API error: ${response.status} ${response.statusText}`);
    }
    ...
    if (input.stream) {
      ...
      const estimatedTokensOut = Math.max(1, Math.floor(estimatedTokensIn * 0.1));
    ...
    if (data.error) {
      throw new Error(data.error.message ?? "Local provider API error");
    }
    ...
    if (input.endpointType === "embeddings") {
      ...
      tokensOut = data.usage?.completion_tokens ?? Math.max(1, Math.floor(tokensIn * 0.1));
    }
